...
int WINAPI LPR_CheckYOLO(int* ptr, char* data, int width, int height, char* resultstr)
{
	...
	for (int n = 0; n < classIds.size(); n++)
	{
		...
		//if (confidence[n] > core->m_detectThreshold)
		{
			...
			//for (int k = 0; k < core->m_prePlateCount; k++)
			//{
			//	if (strcmp(core->m_prePlateInfo[k].m_plateDigit, result.m_plateDigit) == 0)
			//	{
			//		//LogWrite("중복 = %s\n", result.m_plateDigit);
			//		continue; // 중복
			//	}
			...
			//int found = 0;
			//for (int k = 0; k < core->m_resultCount; k++)
			//{
			//	if (strcmp(result.m_plateNo, core->m_result[k].m_plateNo) == 0)
			//	{
			//		SystemTimeToFileTime(&ctm, &core->m_result[core->m_resultCount].m_time);
			//		core->m_result[core->m_resultCount].m_rect = rect;
			//		core->m_result[core->m_resultCount].m_tag = 0;
			//		core->m_result[k].m_count++;
			//		result = core->m_result[k];
			//		found = 1;
			//		break;
			//	}
			//	else if (strcmp(result.m_plateNo, core->m_result[k].m_plateNo) == 0)
			//	{
			//		found = 2;
			//	}
			//}
			...
			//	core->m_result[core->m_resultCount] = result;
			//	core->m_result[core->m_resultCount].m_count = 1;
			//	core->m_result[core->m_resultCount].m_type = id;
			//	core->m_result[core->m_resultCount].m_rect = rect;
			//	core->m_result[core->m_resultCount].m_tag = 0;

			//	SystemTimeToFileTime(&ctm, &core->m_result[core->m_resultCount].m_time);
			//	result = core->m_result[core->m_resultCount];
...

			{
				...
				//char fileName[100];
				//sprintf(fileName, "result\\%4.4d%2.2d%2.2d-%2.2d%2.2d%2.2d-%3.3d-%d-%s.jpg",
				//	ctm.wYear, ctm.wMonth, ctm.wDay,
				//	ctm.wHour, ctm.wMinute, ctm.wSecond, ctm.wMilliseconds,
				//	id,
				//	result.m_plateNo);
				//imwrite(fileName, vimg);

				//LogWrite("%4.4d%2.2d%2.2d-%2.2d%2.2d%2.2d-%3.3d %d %s %d %d %d %d %d %d\n",
				//	ctm.wYear, ctm.wMonth, ctm.wDay,
				//	ctm.wHour, ctm.wMinute, ctm.wSecond, ctm.wMilliseconds,
				//	id,
				//	result.m_plateNo,
	...
	//ULARGE_INTEGER t1, t2;
	//ULONGLONG diff;
	//SystemTimeToFileTime(&ctm, &cft);
	//memcpy(&t1, &cft, sizeof(t1));

	//count = 0;
	//result[0] = 0;
	//char rbuff[100];

	//for (int n = 0; n < core->m_resultCount; n++)
	//{
	//	memcpy(&t2, &core->m_result[n].m_time, sizeof(t1));
	//	diff = (t1.QuadPart < t2.QuadPart) ? (t2.QuadPart - t1.QuadPart) : (t1.QuadPart - t2.QuadPart) / 10000;

	//	if (diff > 500 || core->m_result[n].m_count > 4)
	//	{
	//		if (count > 0)
	//			strcat(result, "$");
	
			//sprintf(rbuff, "LPR;%s;%d;%d;%d;%d;%d",
			//	core->m_result[n].m_plateNo, core->m_result[n].m_count,
			//	core->m_result[n].m_rect.x, core->m_result[n].m_rect.y, core->m_result[n].m_rect.width, core->m_result[n].m_rect.height);
			//strcat(resultstr, rbuff);

	//		{
				//Mat vimg = img.clone();
				//rectangle(vimg, core->m_result[n].m_rect, Scalar(0, 0, 255), 1);

				//char fileName[100];
				//SYSTEMTIME curr;
				//FileTimeToSystemTime(&core->m_result[n].m_time, &curr);
				//sprintf(fileName, "result\\%4.4d%2.2d%2.2d-%2.2d%2.2d%2.2d-%3.3d-%d-%s.jpg",
				//	curr.wYear, curr.wMonth, curr.wDay,
				//	curr.wHour, curr.wMinute, curr.wSecond, curr.wMilliseconds,
				//	core->m_result[n].m_type,
				//	core->m_result[n].m_plateNo);
				//imwrite(fileName, vimg);
				//vimg.release();
	//			LogWrite("%s\n", core->m_result[n].m_plateNo);
	//		}

	//		core->m_prePlateInfo[core->m_prePlateCount].m_time = core->m_result[n].m_time;
	//		strcpy(core->m_prePlateInfo[core->m_prePlateCount].m_plateNo, core->m_result[n].m_plateNo);
	//		strcpy(core->m_prePlateInfo[core->m_prePlateCount].m_plateDigit, core->m_result[n].m_plateDigit);
	//		core->m_prePlateCount++;

	//		count++;

	//		core->m_result[n].m_tag = -9;
	//	}
	//}
	...
	//for (int n = 0; n < core->m_resultCount; n++)
	//{
	//	if (core->m_result[n].m_tag == 0)
	//		temp[tempCount++] = core->m_result[n];
	//}

	//core->m_resultCount = 0;
	//for (int n = 0; n < tempCount; n++)
	//	core->m_result[core->m_resultCount++] = temp[n];

	///////////////////////////////////////////////////////////////////////////////
	/////  중복
	///////////////////////////////////////////////////////////////////////////////
	//bool bdiff = false;
	//for (int n = 0; n < core->m_prePlateCount; n++)
	//{
	//	memcpy(&t2, &core->m_prePlateInfo[n].m_time, sizeof(t1));
	//	diff = (t1.QuadPart < t2.QuadPart) ? (t2.QuadPart - t1.QuadPart) : (t1.QuadPart - t2.QuadPart) / 10000;
	//		
	//	if (diff > 20000) // 20초이내 동일 번호판 pass
	//	{
	//		bdiff = true;
	//		core->m_prePlateInfo[n].m_plateNo[0] = 0;
	//	}
	//}
	...
	//	for (int n = 0; n < core->m_prePlateCount; n++)
	//	{
	//		if (core->m_prePlateInfo[n].m_plateNo[0] == 0)
	//			continue;

	//		ptemp[tempCount++] = core->m_prePlateInfo[n];
	//	}

	//	core->m_prePlateCount = 0;
	//	for (int n = 0; n < tempCount; n++)
	//		core->m_prePlateInfo[core->m_prePlateCount++] = ptemp[n];
	//}

	/////////////////////////////////////////////////////////////////////////////

	//if (count > 0)
	//	LogWrite("Result=%s", result);
