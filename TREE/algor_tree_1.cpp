...
public:
	...
	int num_of_child;
	void input_leftchild(int left)
	{
		this->leftchild = new node(left);
	}
	void input_rightchild(int right)
	{
...
void traversal(node* currentnode, int& child)
{
	if (currentnode->show_left()->show_data() != -1)
	{
		child++;
		traversal(currentnode->show_left(), child);
	}
	if (currentnode->show_right()->show_data() != -1)
	{
		child++;
		traversal(currentnode->show_right(), child);
	}
	else child++;
}
